...
import { Container, Row, Col, ProgressBar, Card } from 'react-bootstrap';
  ...
  function Skills() {
    ...
    const toolsAndSoftSkills = [
      { name: 'Canva', category: 'Design Tool', icon: 'fas fa-paint-brush', color: '#00c4cc' },
      { name: 'VS Code', category: 'Code Editor', icon: 'fas fa-code', color: '#0078d7' },
      { name: 'Team Management', category: 'Soft Skill', icon: 'fas fa-users', color: '#0d6efd' },
      { name: 'Leadership', category: 'Soft Skill', icon: 'fas fa-chess-king', color: '#ffc107' },
    ];
    ...
    return (
      <section id="skills" className="skills-section section-padding section-with-bg-logo">
        <Container>
              ...
              <Col md={4} lg={3} key={idx} className="d-flex">
                <Card className="skills-card text-center h-100">
                  <Card.Body className="d-flex flex-column">
                    ...
                    <Card.Title as="h5" className="mt-3">{skill.name}</Card.Title>
                    <Card.Text as="p" className="flex-grow-1">{skill.description}</Card.Text>
                    <div className="mt-auto w-100">
                      <ProgressBar now={animated ? skill.level : 0} className="mb-2" style={{ height: '8px' }} />
          ...
          <Row className="g-4 justify-content-center">
            {toolsAndSoftSkills.map((tool, idx) => (
              <Col md={3} xs={6} key={idx} className="d-flex">
                <Card className="skills-card text-center h-100">
                  <Card.Body>
                    <i className={`${tool.icon} fa-2x skills-icon`} style={{ color: tool.color }}></i>
